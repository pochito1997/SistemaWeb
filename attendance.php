	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Ya has registrado entrada';
				}
				else{
					//updates
					...
					if($conn->query($sql)){
						$output['message'] = 'Puntual: '.$row['firstname'].' '.$row['lastname'];
					}
					else{
						$output['error'] = true;
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'No se ha registrado entrada.';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Ya has registrado salida';
					}
					else{
			...
			$output['error'] = true;
			$output['message'] = 'ID empleado no Encontrado';
		}
